...
type
  TFrmCadGeralProd = class(TForm)
    ...
    QryInsertDadosCódigo: TIntegerField;
...
begin
  ...
  if LblDesc.Caption = 'Descrição Grupo' then
  begin
    ...
    QryBuscaDados.open;
    DBConsutaCadGeralProd.Columns[0].Title.Caption := 'Código';
    DBConsutaCadGeralProd.Columns[1].Title.Caption := 'Descrição';
    EdtDescricao.Clear;
    MessageDlg('Grupo de Produtos Cadastrado Com Sucesso.', mtInformation, [mbOK], 0);
  end
  else
  if LblDesc.Caption = 'Descrição SubGrupo' then
  begin
  ...
  if LblDesc.Caption = 'Descrição Família' then
  begin
    ...
    DBConsutaCadGeralProd.Columns[1].Title.Caption := 'Descrição';
    EdtDescricao.Clear;
    MessageDlg('Família de Produtos Cadastrada Com Sucesso.', mtInformation, [mbOK], 0);
  end;
end;
